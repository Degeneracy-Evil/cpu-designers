// ============================================================================
// soc_fixture.svh — Shared system_top test fixture
//
// This file is included via `include "support/soc_fixture.svh" inside each
...
generate if (`SIMU_USE_DDR == 0) begin: sim_ram_tb
    ...
    task uart_send_byte;
        ...
        begin
            ...
            // Flow control: leave headroom in the receive FIFO before sending
            // the next byte. Use while+@(posedge clk) for XSim robustness.
...
// ----------------------------------------------------------------
// finish_framework_test — common result handling for program tests.
// ----------------------------------------------------------------
task finish_framework_test;
    input integer max_cycles;
    reg completed;
    reg [31:0] passed;
    reg [31:0] total;
    ...
    begin
        pass_count = 0;
        fail_count = 0;
        wait_test_report(max_cycles, completed);

        if (!completed)
            $fatal(1, "framework report timeout after %0d cycles", max_cycles);

        read_reg(5'd28, passed);
        read_reg(5'd29, total);
        read_reg(5'd30, first_fail);
