...
module tb_cpu_mem_contract_unit;
    ...
    wire [31:0] mem_vaddr, phys_req_wdata, mem_pc, mem_inst;
    ...
    task allow_access; begin
        @(negedge clk);mem_access_ready=1;
        @(posedge clk);#1;
    ...
    initial begin
        mem_valid=0;trap_enter=0;mem_access_ready=0;
        ...
        allow_access();complete_phys(32'h12345678);
        check(mem_done&&mem_wb_bus.wb_data==32'h12345678,"LR returns loaded value");
        finish_op();

        start_op(MEM_SC,32'h80004000,32'hdeadbeef,`AXI_SIZE_WORD,5'b00011);
        allow_access();
        check(phys_req_valid&&phys_req_write&&phys_req_wdata==32'hdeadbeef,
              "successful SC goes directly to physical write");
        complete_phys(0);
