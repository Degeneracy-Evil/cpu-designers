...
```bash
python tools/mk.py --top <tb_top.v> --top-module <tb_module>
```

### 1.3 波形生成

```bash
python tools/mk.py --top <tb_top.v> --top-module <tb_module> --define GENERATE_WAVE
gtkwave <wave_file>.vcd
```

## 2. 统一命令规范（mk.py）

项目内所有仿真命令统一通过 `mk.py` 执行：

```bash
# 仅编译
python tools/mk.py --top <tb_top.v> --top-module <tb_module> --compile-only

# 编译 + 运行
python tools/mk.py --top <tb_top.v> --top-module <tb_module>

# 仅运行（复用 build 目录已有产物）
python tools/mk.py --top <tb_top.v> --top-module <tb_module> --run-only

# 宏开关（示例：生成波形）
python tools/mk.py --top <tb_top.v> --top-module <tb_module> --define GENERATE_WAVE

# 预检命令（不实际执行编译）
python tools/mk.py --top <tb_top.v> --top-module <tb_module> --compile-only --dry-run
```
