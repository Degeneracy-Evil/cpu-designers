`timescale 1ns / 1ps
module tb_regression_reg_sfence_during_walk;
    localparam integer EXPECTED_TOTAL = 2;
    localparam integer SIM_CYCLES    = 200000;
    reg clk; reg reset; reg [4:0] rf_addr;
    wire [31:0] rf_data, if_pc, if_inst, id_pc, id_inst, exe_pc, exe_inst, mem_pc, mem_inst, wb_pc, wb_inst, display_state;
    wire [31:0] cpu_HADDR, cpu_HWDATA, cpu_HRDATA;
    wire [1:0]  cpu_HTRANS; wire cpu_HWRITE, cpu_HREADY, cpu_HRESP, cpu_HMASTLOCK;
    wire [2:0]  cpu_HSIZE, cpu_HBURST; wire [3:0] cpu_HPROT;
    wire timer_irq, plic_eip, clint_mtip, clint_msip; integer pass_count, fail_count;
    core_top dut(.clk(clk), .reset(reset), .rf_addr(rf_addr), .rf_data(rf_data),
    ...
    wire [15:0] gpio_io;
    ahb_lite_bus #(.ADDR_WIDTH(32), .DATA_WIDTH(32), .SLAVE_NUM(5), .MEM_DEPTH(8192), .WAIT_STATES(0), .GPIO_NUM(16), .UART_FREQ(100))
    u_bus (.HCLK(clk), .HRESETn(~reset), .HADDR(cpu_HADDR), .HTRANS(cpu_HTRANS), .HWRITE(cpu_HWRITE),
        .HSIZE(cpu_HSIZE), .HBURST(cpu_HBURST), .HPROT(cpu_HPROT), .HMASTLOCK(cpu_HMASTLOCK),
        .HWDATA(cpu_HWDATA), .HRDATA(cpu_HRDATA), .HREADY(cpu_HREADY), .HRESP(cpu_HRESP),
        .o_timer_irq(timer_irq), .o_gpio_irq(), .o_uart_irq(), .o_spi_irq(),
        .o_plic_eip(plic_eip), .o_clint_mtip(clint_mtip), .o_clint_msip(clint_msip),
        .io_gpioPin(gpio_io), .i_uart_rx(1'b1), .o_uart_tx(), .o_spiMosi(), .i_spiMiso(1'b0),
    ...
    initial begin
        ...
        $display(""); $display("--- Regression sfence_during_walk Results ---");
        ...
        rf_addr = 5'd30; #1;
        if (rf_data === 32'd0) begin pass_count = pass_count + 1; $display("  PASS first_fail_id = 0"); end
        else begin fail_count = fail_count + 1; $display("  FAIL first_fail_id = %0d", rf_data); end
        $display(""); $display("========================================");
        $display("Regression sfence_during_walk summary"); $display("pass=%0d fail=%0d", pass_count, fail_count);
