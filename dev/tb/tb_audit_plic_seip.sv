...
module tb_audit_plic_seip;
    ...
    reg [31:0] _val;
    initial begin
        // Force PLIC source 4 (GPIO) high to trigger interrupt pending
        // This sets r_pending[4] in the PLIC, which with context 1 enabled
        // and priority > threshold, asserts o_eip[1] → ext_seip
        force u_soc.plic_src_irq[4] = 1'b1;

        pass_count = 0; fail_count = 0; repeat (SIM_CYCLES) @(posedge clk);

        $display(""); $display("--- Audit plic_seip Results ---");
        ...
        $display(""); $display("========================================");
        $display("Audit plic_seip summary"); $display("pass=%0d fail=%0d", pass_count, fail_count);
