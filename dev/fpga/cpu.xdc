...
# ============================================================
# NOTE: Pin assignments are board-specific. The pins below are
# placeholders for the Artix-7 A200T FBG676 board. Adjust to
# match your actual PCB routing. The MIG IP generates its own
# internal timing constraints via XDC — do NOT override them.
#
# Key MIG constraints (auto-generated by IP):
#   - sys_clk_i  : 100MHz input clock (period 10ns)
#   - clk_ref_i  : 200MHz reference clock (period 5ns)
#   - ui_clk      : 100MHz fabric clock output
#   - DDR3 interface: 800Mbps data rate, 400MHz DDR clock
#
# The clk_wiz_0 also generates constraints automatically.

# DDR3 clock period constraint for sys_clk_i (drives MIG sys_clk_i)
# This is the same 100MHz clock as 'clk' — already constrained above.
# MIG's internal MMCM will generate ui_clk from this.

# DDR3 IOSTANDARD settings (SSTL15 for Artix-7 DDR3)
...
# Internal clock constraint for MIG ui_clk (100MHz fabric clock)
# MIG generates this from sys_clk_i via internal MMCM.
# This constraint is informational — MIG's own XDC handles the timing.
# create_generated_clock -name mig_ui_clk -source [get_pins u_ahb_lite_bus/u_ddr3_bridge_wrapper/u_mig/u_ddr3_infrastructure/u_mmcm_i/CLKIN1] [get_pins u_ahb_lite_bus/u_ddr3_bridge_wrapper/u_mig/u_ddr3_infrastructure/u_mmcm_i/CLKOUT0]
