...
if {{ [file exists "{mig_ip_dir}"] }} {{
    ...
    add_files -fileset sim_1 -norecurse [file join "{mig_ip_dir}" bd_soc_mig_7series_0_1_mig.v]
...
class Operations:
    ...
    def _patch_prj_and_rerun(
        ...
        # Re-run xvlog, xelab, xsim manually
        vivado_bin = os.environ.get(
            "XILINX_VIVADO", r"C:\Xilinx\Vivado\2018.3"
        )
        xvlog = str(Path(vivado_bin) / "bin" / "xvlog.bat")
        xelab = str(Path(vivado_bin) / "bin" / "xelab.bat")
        xsim = str(Path(vivado_bin) / "bin" / "xsim.bat")

        output_parts: list[str] = []

        # xvlog — use just the prj filename since cwd is xsim_dir.
        # Don't fail on non-zero rc — Vivado's internal flow also proceeds
        # past xvlog errors (e.g. procedural assignment to wire in ahb_sys_status.sv).
        try:
            r = subprocess.run(
                [xvlog, "--relax", "-prj", prj_path.name],
                cwd=str(xsim_dir),
                capture_output=True,
                text=True,
                timeout=120,
            ...
            if r.returncode != 0:
                logger.warning("xvlog had errors (rc=%d) but proceeding to xelab", r.returncode)
        except Exception as e:
            logger.error("xvlog failed: %s", e)
            return ExecuteResult(output=str(e), success=False, timed_out=False, duration=0.0)

        # xelab
        tb = task.tb
        # xelab -d takes KEY=VALUE as a single argument.
        # On Windows, cmd.exe splits on '=', so we must quote: -d "KEY=VALUE"
        define_flags: list[str] = []
        for k, v in (task.verilog_defines or {}).items():
            define_flags.extend(["-d", f'"{k}={v}"'])
        ...
        xelab_base_args = [
            ...
            *define_flags,
            ...
            "-L", "xil_defaultlib",
            "-L", "ahblite_axi_bridge_v3_0_13",
            "-L", "unisims_ver",
            "-L", "unimacro_ver",
            "-L", "secureip",
            "-L", "xpm",
            "-L", "ahblite_axi_bridge_0_vhd",
        ...
        # Try xelab with progressively safer settings if XSIM 43-3356 occurs.
        # This is a known Vivado 2018.3 Windows bug where multi-threaded code
        # generation races on xsim.type file locking.
        xelab_attempts = [
            ("mt2_debug_typical", ["-debug", "typical", "-mt", "2"]),
            ("mtoff_debug_off",  ["-debug", "off",    "-mt", "off"]),
            ("mtoff_debug_typical",["-debug", "typical", "-mt", "off"]),
        ]
        xelab_ok = False
        for attempt_name, extra_args in xelab_attempts:
            logger.info("xelab attempt: %s", attempt_name)
            try:
                r = subprocess.run(
                    xelab_base_args + extra_args,
                    cwd=str(xsim_dir),
                    capture_output=True,
                    text=True,
                    timeout=600,
                ...
                if r.returncode == 0:
                    xelab_ok = True
                    break
                # XSIM 43-3356: file locking race — kill lingering processes and retry
                if "XSIM 43-3356" in (r.stderr + r.stdout):
                    logger.warning(
                        "xelab hit XSIM 43-3356 (attempt %s), killing processes and retrying",
                        attempt_name,
                    )
                    # Kill lingering Vivado sim processes via taskkill (Windows)
                    for pname in ("xsimk.exe", "xelab.exe", "xsim.exe"):
                        subprocess.run(
                            ["taskkill", "/F", "/IM", pname],
                            capture_output=True,
                        )
                    ...
                    _time.sleep(3)
                    # Clean stale xsim.dir snapshot to avoid corrupted state
                    snap_dir = xsim_dir / "xsim.dir" / f"{tb}_behav"
                    if snap_dir.exists():
                        import shutil
                        shutil.rmtree(snap_dir, ignore_errors=True)
                    continue
                # Any other xelab error → fail immediately
                logger.error("xelab failed (rc=%d)", r.returncode)
                ...
                logger.error("xelab failed: %s", e)
                return ExecuteResult(output=str(e), success=False, timed_out=False, duration=0.0)

        if not xelab_ok:
            logger.error("xelab failed after all attempts (XSIM 43-3356)")
            return ExecuteResult(
                output="\n".join(output_parts),
                success=False,
                timed_out=False,
        ...
        # xsim
        # BUG-55d: Increased timeout from 600s to 3600s (1 hour).
        # Full system simulation with CPU + MIG + DDR3 model needs much
        # longer than 10 minutes to reach MIG calibration (~200µs sim time).
        xsim_timeout = 3600
        try:
            r = subprocess.run(
                [
                ...
                capture_output=True,
                text=True,
                timeout=xsim_timeout,
            )
            output_parts.append(r.stdout + r.stderr)
        except Exception as e:
            logger.error("xsim failed: %s", e)
            return ExecuteResult(output=str(e), success=False, timed_out=False, duration=0.0)

        # Read simulate.log for test results
        sim_log = xsim_dir / "simulate.log"
        if sim_log.exists():
            output_parts.append(sim_log.read_text(encoding="utf-8", errors="replace"))

        success = "ALL TESTS PASSED" in "\n".join(output_parts) or "PASS" in "\n".join(output_parts)
        return ExecuteResult(
            output="\n".join(output_parts),
            success=success,
            timed_out=False,
