...
```bash
# 全回归
python -m tools.vivado_cli --tasks config/tasks.yaml -batch "*" -create -sim

# 按类别
python -m tools.vivado_cli --tasks config/tasks.yaml -batch "mmu_*" -create -sim

# 仅仿真 (复用已有会话)
python -m tools.vivado_cli --tasks config/tasks.yaml -batch "*" -sim
```
