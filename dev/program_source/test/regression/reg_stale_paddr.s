# ============================================================
# regression/reg_stale_paddr.s — Bug 8 regression
...
# Test 1: Fill 3 pages then verify all correct (no stale paddr corruption)
t1: la x5, ms; sw x1, 0(x5); sw x0, 4(x5)
    jal x1, setup_identity_map; jal x1, enable_sv32
    la x5, s1; csrw mepc, x5; li x5, 0x880; csrw mstatus, x5; mret
s1: la x14, td; lw x15, 0(x14); li x16, 0xDEADBEEF; bne x15, x16, _f1
    la x14, td2; lw x15, 0(x14); li x16, 0xCAFEBABE; bne x15, x16, _f1
    li x14, 1; j _d1
_f1: li x14, 0
_d1: la x15, mr; sw x14, 0(x15); ecall

# Test 2: Cross-page verification (re-read first page after accessing second)
...
s2: la x14, td; lw x15, 0(x14); li x16, 0xDEADBEEF; bne x15, x16, _f2
    ...
    la x14, td; lw x15, 0(x14); li x16, 0xDEADBEEF; bne x15, x16, _f2
    li x14, 1; j _d2
_f2: li x14, 0
