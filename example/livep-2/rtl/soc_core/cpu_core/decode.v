...
//    description: 通过对指令进行分类�?�译码，输出通用寄存器地�????、CSR寄存器地�????、CSR写使能以及�?�线数据�????
//                 数据包中包括寄存器数据�?�GRF写使能�?�alu控制信号、alu操作数�?�低位乘积�?�高位乘积�?�商、余数�?? 
//                 乘除法符号标志�?�目的寄存器地址、load标志、store标志、读写宽度标志�?�CSR指令标志、乘除法相关操作标志
//
//-----------------------------------------------
`timescale 1ns / 1ps
`include "./riscv.h"
...
//-----{指令分类}begin
    ...
    assign w_slt_1  = w_instSLT_1   | w_instSLTI_1 ;                        // 有符号小于置�????
    assign w_sltu_1 = w_instSLTU_1  | w_instSLTIU_1;                        // 无符号小于置�????
    assign w_and_1  = w_instAND_1   | w_instANDI_1
                    | w_instCSRRC_1 | w_instCSRRCI_1;                       // 逻辑�????
    assign w_or_1   = w_instOR_1    | w_instORI_1
                    | w_instCSRRS_1 | w_instCSRRSI_1    ;                   // 逻辑�????
...
//-----{输出信号生成}begin
    // ALU两个源操作数和控制信�????
    wire [ 1:0] w_mulDivSign_2;         // 乘除法符号标�????
    wire [ 4:0] w_rdValue_5;            // 目的寄存器地�????
    wire [31:0] w_imm_32;               // 立即�????
    wire [31:0] w_csrOper2_32;          // csr指令操作�????2
