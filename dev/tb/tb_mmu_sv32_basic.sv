...
module tb_mmu_sv32_basic;
        ...
        forever begin
            ...
            if ((cycle_cnt >= 3820 && cycle_cnt <= 3960) ||
                ...
                $display("[t=%0d] pc=0x%08h priv=%0d trap=%0b mret=%0b mstatus=0x%08h mepc=0x%08h",
                    cycle_cnt, exe_pc, dut.priv_mode, dut.trap_enter_valid, dut.dec_is_mret,
                    dut.csr_mstatus, dut.csr_mepc);
            end
            ...
            if (dut.trap_enter_valid && cycle_cnt > 100) begin
                ...
                if (trap_count <= 6) begin
                    $display("[t=%0d] >>> TRAP #%0d: exe_pc=0x%08h mepc_csr=0x%08h mstatus=0x%08h priv=%0d",
                        cycle_cnt, trap_count, exe_pc, dut.csr_mepc, dut.csr_mstatus, dut.priv_mode);
                end
            end

            // Detect mret execution
            if (dut.dec_is_mret && cycle_cnt > 100 && cycle_cnt <= 5000) begin
                $display("[t=%0d] >>> MRET: exe_pc=0x%08h mepc_csr=0x%08h mstatus=0x%08h priv=%0d",
                    cycle_cnt, exe_pc, dut.csr_mepc, dut.csr_mstatus, dut.priv_mode);
            end
        end
    end
    ...
    initial begin
        ...
        $display("    mepc   = 0x%08h", dut.csr_mepc);
        $display("    mtvec  = 0x%08h", dut.csr_mtvec);

        // Read x5-x6, x10, x14, x22-x24 for debug
        rf_addr = 5'd10; #1;
        $display("    x10    = 0x%08h (%0d)", rf_data, rf_data);
        rf_addr = 5'd22; #1;
        $display("    x22(mcause)= 0x%08h", rf_data);
        rf_addr = 5'd23; #1;
        $display("    x23(mepc)  = 0x%08h", rf_data);
        rf_addr = 5'd24; #1;
        $display("    x24(mtval) = 0x%08h", rf_data);
