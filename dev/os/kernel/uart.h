#ifndef _OS_KERNEL_UART_H
#define _OS_KERNEL_UART_H

/*
 * Kernel-mode NS16550A UART driver (S-mode, direct MMIO).
 * Direct copy of dev/program_source/lib/include/uart.h with sys.h
 * replaced by memlayout.h (provides UART_BASE) and types.h (via
 * memlayout.h) for fixed-width integer types.
 *
 * Register map: byte offsets mapped to word-aligned APB4 addresses
 * (reg N at byte offset N -> APB4 offset N*4).
 */

#include "memlayout.h"   /* UART_BASE, types.h */
...
/*  NS16550A byte offset -> APB4 word offset (x4)                     */
